...
public class FileReaderGenerator {
  ...
  private String addAttributeAssignments(String baseContent) {
    ...
    String floatAttrAssignmentTemplate = templateReader.readFile("FileReader-FloatAttributeAssignmentTemplate.tmpl");
    String linkMethodAssignmentTemplate = templateReader.readFile("FileReader-RetrOrCreaTemplate.templ");
    ...
    for (int i = 0; i < nounDescriptor.attributeNames.size(); i++) {
      ...
      if (type.equals("float")) {
        String attrDefinition = stringHandler.replaceAll(floatAttrAssignmentTemplate, "%attr%", name);
        attrDefinition = stringHandler.replaceAll(attrDefinition, "%lineNr%", Integer.toString(i));
        result = stringHandler.injectBefore(result, "                        // end:attribute-assignments", attrDefinition);
        continue;
      }
      ...
      attrDefinition = stringHandler.replaceAll(attrDefinition, "%lineNr%", Integer.toString(i));
      result = stringHandler.injectBefore(result, "                        // end:attribute-assignments", attrDefinition);
      String retrieveOrCreate = stringHandler.replaceAll(linkMethodAssignmentTemplate, "%attr%", name);
      result = stringHandler.injectBefore(result, "    // end:retrieveOrCreate", retrieveOrCreate); // Injecting retrieveOrCreate
    }
