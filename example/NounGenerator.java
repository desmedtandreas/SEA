...
public class NounGenerator {
  ...
  public String addAttributeDefinitions(String baseContent) {
    ...
    String attrListDefTemplate = templateReader.readFile("AttributeListDefinitionTemplate.tmpl");

    for (int i=0; i < nounDescriptor.attributeTypes.size(); i++) {
      String template = nounDescriptor.attributeTypes.get(i).contains("List") ? attrListDefTemplate : attrDefTemplate;
      String attrDefinition = stringHandler.replaceAll(template, "%type%", nounDescriptor.attributeTypes.get(i));
      attrDefinition = stringHandler.replaceAll(attrDefinition, "%name%", nounDescriptor.attributeNames.get(i));
      result = stringHandler.injectBefore(result, "  // end:attribute-defin", attrDefinition);
    }
  ...
  public String addGetSetMethods(String baseContent) {
    ...
    for (int i=0; i < nounDescriptor.attributeTypes.size(); i++) {
      ...
      result = stringHandler.injectBefore(result, "  // end:getset-meth", attrGetSet);
    }
