...
public class OrderFileReader {
            ...
            while (aLine != null) {
                ...
                if (aLine != null) {
                    ...
                    if (lineParts.length == 3) {
                        ...
                        instance.setOrderId(lineParts[0]);
                        instance.setLogoUrl(lineParts[1]);
                        String mCustomerId = lineParts[2];
                        Customer mCustomer = retrieveOrCreateCustomer(mCustomerId);
                        instance.setCustomer(mCustomer);
    ...
    private Customer retrieveOrCreateCustomer(String aCustomerId) {
        CustomerList mCustomerRepository = repository.getCustomerList();

        Customer result = mCustomerRepository.getMember(aCustomerId);
        if (result != null) {
            return result;
        }

        Customer mCustomer = new Customer();
        mCustomer.setCustomerId(aCustomerId);
        mCustomerRepository.addMember(mCustomer);
